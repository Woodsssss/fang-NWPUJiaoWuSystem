...
export default {
  ...
  methods: {
    ...
    async getAdminList(search) {
      ...
      if (search) {
        if (this.selected === '账号') {
          query = { user_account:this.input, user_name: undefined}

        } else if (this.selected === '姓名') {
          query = { user_account:undefined,   user_name: this.input}
          
        }
      }

      if(typeof query.user_account != "undefined"){
        console.log("query:"+query.user_account+query.user_name)
        const { data: res } = await this.$http.get('/edustaff/staffadmin/info-account/'+ query.user_account)
        if (res.code !== 200) return this.$message.error('获取用户列表失败')
        this.adminList = res.staffAdmin
        this.total = res.total
        // console.log(res)
      } 
      else if(typeof query.user_name!="undefined"){
        console.log("query:"+query.user_account+query.user_name)
        const { data: res } = await this.$http.get('/edustaff/staffadmin/info-name/'+ query.user_name)  
        if (res.code !== 200) return this.$message.error('获取用户列表失败')
        this.adminList = res.staffAdmin
        this.total = res.total
        // console.log(res)
        ...
        console.log("query:"+query.user_account+query.user_name)
        const { data: res } = await this.$http.get('/edustaff/staffadmin/info/')
        if (res.code !== 200) return this.$message.error('获取用户列表失败')
    ...
    addAdmin() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return
        // 校验正确则发起请求
        const { data: res } = await this.$http.post('/edustaff/staffadmin/save',this.addForm)
    ...
    editAdmin() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.post(
          '/edustaff/staffadmin/update',
          {id:this.editForm.id, aname: this.editForm.aname}
        )
