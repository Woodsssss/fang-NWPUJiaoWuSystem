        ...
        <el-row>
          <el-col span="12">
            <el-form-item label="学号" prop="scode">
              <el-input v-model="addForm.scode" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="课程号" prop="ccode">
              <el-input v-model="addForm.ccode" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
  ...
  methods: {
    async getstudent() {
      ...
      if(typeof query.ccode!="undefined"){
        const { data: res } = await this.$http.post('eduacademic/academicgrade/info-ccode/'+ query.ccode)
        // console.log(res)
        if (res.code !== 200) return this.$message.error('获取学生列表失败')
        this.studentlist = res.academicGrade
        this.total = res.total
      }else{
        const { data: res } = await this.$http.post('eduacademic/academicgrade/info', {id:this.id})
        // console.log(res)
        if (res.code !== 200) return this.$message.error('获取学生列表失败')
        this.studentlist = res.academicGrade
        this.total = res.total
    ...
    async showAddDialog(row) {
      ...
      this.addForm.scode = row.student.scode
      this.addForm.sname = row.student.sname
      this.addForm.ccode = row.course.ccode
      this.addForm.cname = row.course.cname
    },
    // 录入成绩
    addScore() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.post(
          'eduacademic/academicgrade/update',
          {
            id: this.addForm.id,
            scode: this.addForm.scode,
            ccode: this.addForm.ccode,
            grades: this.addForm.score,
          }
        )
